...
<div class="form" style="display:block; position:relative; z-index:0">
    <form method="POST" action="{{ route('changePSW') }}">
        ...
        <!-- email field -->
        <div class="fieldset">
            <input id="email_register" type="email" placeholder="e-mail" name="email" required>
        ...
        <div class="fieldset">
            <input type="password" placeholder="répéter le nouveau mot de passe" name="new_password_confirmation" required>
        </div>

        <!-- submit button -->
        <button name="submit" type="submit">Changer le mot de passe</button>
    </form>
</div>
<!-- if user disconnected display : -->
@else
<p class="already_log">Utilisateur non connecté : vous devez être connecté pour voir cette page</p>
@endif
@endsection
